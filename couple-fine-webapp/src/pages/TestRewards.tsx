...
export const TestRewards: React.FC = () => {
  const { state, createReward, claimReward, deleteReward } = useTestApp();
  ...
  const totalPenalties = Math.round(state.violations.reduce((sum, violation) => sum + violation.amount, 0) / 10000); // Convert to 만원
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('🧪 TEST: 보상 생성 시도:', rewardData);
      
      const { error } = await createReward(rewardData);
  ...
  const handleClaimReward = async (rewardId: string, rewardTitle: string) => {
    try {
      console.log('🧪 TEST: 보상 달성 시도:', rewardId);
  ...
  const handleDeleteReward = async (rewardId: string, rewardTitle: string) => {
    ...
    try {
      console.log('🧪 TEST: 보상 삭제 시도:', rewardId);
